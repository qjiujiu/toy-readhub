...
**book 表再多加一个字段 group_id，相同图书（具有相同的作者和书名） 具有相同的 group_id 值。**

**在插入book 表之前先查找该图书是否已经存在（用 author 和 title 联合查找）**

* **不存在，直接插入，book_id 保持自增，该书 group_id 的值等于 book_id 值**

  * **还要插入 book_location, book_inventory 表**
* **存在，记录相同图书的 group_id 值，直接插入，book_id 保持自增，该书 group_id 的值等于相同图书的 group_id 值**

  * **跳过book_location 表，在 book_inventory 表添加库存**

**各表字段变化：**

**图书表 book 字段：bid, **group_id**, title, author, isbn, abstract, tags                           （添加 group_id 字段）

**图书位置表 book_location 字段：loc_id, **group_id**, area, floor, warehouse_name   （原本book_id 改为 group_id）

**图书库存表 book_inventory 字段：inv_id, **group_id**, warehouse_name, quantity    （原本book_id 改为 group_id）
...
以上方案全部推翻，朝令夕改，没错我是皇帝

### 最终方案

相同的图书具有相同的 isbn，利用这点重新设计数据表

- book 字段：bid, title, author, isbn, abstract, tags
- book_location 字段：loc_id, bid, **isbn**, area, floor, warehouse_name
- book_inventory 字段：inv_id, **isbn**, warehouse_name, quantity
- order 字段：order_id, user_id, book_id, warehouse_name, status, borrow_time, return_time

 book_id（bid）唯一标识一本书（还书时无需身份验证）

**插入逻辑**

在 book 表插入一本书前，先根据 isbn 查看该书是否已经存在

- 不存在，在 book, book_inventory, book_location 表中插入该书
- 存在，跳过 book_location, 在 book 和 book_inventory 表中插入库存记录


**移除 `book_inventory.isbn → books.isbn` 外键**

一旦让 `book_inventory.isbn` → **books.isbn** 做外键，books.isbn 在父表并不唯一，当删除图书表某条记录 `books(bid=31, isbn=... )` 时，外键要定位“被删的这条父行”。但父表中  **isbn 不唯一** ，无法唯一对应，导致删除被拒绝（即便还有别的 `books` 行拥有相同 isbn）

外键必须是唯一的。`book_inventory.isbn` 不能直接外键到 `books.isbn`，因为 `books.isbn` 不是唯一键
