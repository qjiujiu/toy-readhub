...
class SQLAlchemyBookinvRepository(IBookInventoryRepository):
    ...
    def get_inventories_by_isbn(self, isbn: str) -> List[dict]:
        invs = (
            self.db.query(BookInventory)
            .filter(BookInventory.isbn == isbn)
            .order_by(BookInventory.warehouse_name.asc())  # 可选：按校区名排序
            .all()
        )
        return [BookInventoryOut.model_validate(inv).model_dump() for inv in invs]
    ...
    # 根据图书isbn和校区信息获取图书记录
    def get_by_isbn_and_warehouse(self, isbn: str, warehouse_name: str) -> Optional[dict]:
        name = (warehouse_name or "").strip()
        inv = (
            self.db.query(BookInventory)
            .filter(BookInventory.isbn == isbn, BookInventory.warehouse_name == name)
            .one_or_none()
        )
        if not inv:
            return None
    ...
    def update_inventory(self, isbn: str, warehouse_name: str, delta: int = 1) -> Optional[dict]:
        inv = (
            self.db.query(BookInventory)
            .filter(BookInventory.isbn == isbn, BookInventory.warehouse_name == warehouse_name.strip())
            .one_or_none()
        )
        if not inv:
            return None
    ...
    def create_inventory(self, inventory_data: BookInventoryCreate) -> Dict:
        inv = BookInventory(isbn=inventory_data.isbn, warehouse_name=inventory_data.warehouse_name, quantity=1)
        with transaction(self.db):
            self.db.add(inv)
    ...
    # 删除图书库存信息，
    def delete_inventory(self, isbn: str) -> None:
        # 查找库存记录
        inventory = self.db.query(BookInventory).filter(BookInventory.isbn == isbn).first()
        if not inventory:
            raise ValueError(f"Inventory for ISBN {isbn} not found.")  # 如果没有找到库存记录，抛出异常
